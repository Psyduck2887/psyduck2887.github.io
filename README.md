...
- [Anatolo](https://github.com/Lhcfl/hexo-theme-anatolo) —— 深色极简博客主题（本仓库内二次改造）
...
浏览器打开 <http://localhost:4000>，看到的是套了 Anatolo 改造主题的最终效果（访客看到的样子）。

修改文章或主题后，必要时重新运行 `npm run clean && npm run build` 检查最终生成结果。
...
主题配置在 `themes/Anatolo/_config.yml`，样式源码在 `themes/Anatolo/src/scss/`。
